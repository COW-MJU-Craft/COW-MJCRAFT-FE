...
function getOrderViewErrorState(error: unknown): OrderViewErrorState {
  if (error instanceof ApiError) {
    if (error.status === 400) {
      return {
        ...
        description:
          '이메일에 포함된 주문 확인 토큰이 없거나 잘못되었어요. 메일의 링크를 다시 열어주세요.',
      };
    }
    if (error.status === 404) {
      return {
        title: '유효하지 않은 주문 확인 링크예요',
        description:
          '해당 링크로 확인할 수 있는 주문 정보를 찾지 못했어요. 주문 안내 메일을 다시 확인해주세요.',
      };
    }
    if (error.status === 410) {
      return {
        title: '주문 확인 링크가 만료되었어요',
        description: '이메일 링크의 사용 기간이 지났어요.',
      };
    }
  }
  ...
  return {
    ...
    description: '잠시 후 이메일의 주문 안내 링크를 다시 열어주세요.',
  };
}
    ...
    const load = async () => {
      ...
      try {
        ...
        if (
          ...
          toast.error(
            error instanceof Error ? error.message : '주문 정보를 불러오지 못했어요.',
          );
        }
      } finally {
        if (active) {
        ...
        <Reveal>
          <section className="rounded-3xl border border-rose-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-600">
              이메일 링크에 확인 토큰이 없어 주문 정보를 확인할 수 없어요.
              주문 안내 메일의 링크를 다시 열어주세요.
            </p>
            <Link
              to="/"
    ...
    <div className="mx-auto max-w-4xl px-4 py-12">
      <Reveal>
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <h1 className="font-heading text-3xl text-slate-900">주문 정보</h1>
          <p className="mt-2 text-sm text-slate-600">
