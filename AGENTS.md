...
- 진행 중 사용자가 별도 질문을 하면 질문에만 답하고 기존 작업을 멈춘다. 사용자가 명시적으로 재개를 요청하기 전에는 이전 작업을 자동으로 이어가지 않는다.
...
- 전체 테스트(`npm test`), E2E, 다중 뷰포트 Playwright 캡처, 보안·의존성 전체 검사, 저장소 전체 감사처럼 시간·출력·토큰 사용량이 큰 검증은 실행 전에 사용자 승인을 받는다.
- 승인 요청에는 실행할 명령, 검증 목적, 예상 범위를 먼저 제시한다.
- 사용자가 해당 검증을 명시적으로 요청한 경우에는 승인된 것으로 본다.
- 스타일·마크업만 변경한 작업은 기본적으로 변경 diff와 관련 화면만 확인한다. 전체 테스트는 별도 승인 없이는 실행하지 않는다.
